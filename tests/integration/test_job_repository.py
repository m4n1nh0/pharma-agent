...
async def test_create_and_get():
    ...
    job = await repo.create(JobType.DRUG_ANALYSIS, "usr_1", {"drug_name": "Test"})
    assert await repo.get(job.id) is job
...
async def test_mark_running_updates_status():
    repo = InMemoryJobRepository()
    job = await repo.create(JobType.INTERACTIONS, "usr_1", {})
...
async def test_mark_completed_sets_result():
    repo = InMemoryJobRepository()
    job = await repo.create(JobType.PRESCRIPTION, "usr_1", {})
...
async def test_mark_failed():
    repo = InMemoryJobRepository()
    job = await repo.create(JobType.DRUG_ANALYSIS, "usr_2", {})
    await repo.mark_running(job.id)
...
async def test_list_by_user():
    repo = InMemoryJobRepository()
    await repo.create(JobType.DRUG_ANALYSIS, "usr_A", {})
    await repo.create(JobType.DRUG_ANALYSIS, "usr_A", {})
    await repo.create(JobType.DRUG_ANALYSIS, "usr_B", {})
    assert len(await repo.list_by_user("usr_A")) == 2
    assert len(await repo.list_by_user("usr_B")) == 1
    assert len(await repo.list_by_user("usr_Z")) == 0


@pytest.mark.asyncio
async def test_sse_completed_job_delivers_immediately():
    repo = InMemoryJobRepository()
    job = await repo.create(JobType.DRUG_ANALYSIS, "usr_1", {})
    await repo.mark_running(job.id)
